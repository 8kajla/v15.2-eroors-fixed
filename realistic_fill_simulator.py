...
class RealisticFillSimulator:
    ...
    def on_trade_print(
        ...
        fingerprint = f"{token}|{trade_ts:.6f}|{trade_price:.10f}|{trade_size:.10f}|{str(trade_side).upper()}|{transaction_hash}"
        if trade_size <= 0:
            return
        with self._lock:
            # Deduplicate before applying any fill accounting. Stable IDs catch
            # normal replay; the exact payload fingerprint catches duplicate
            # WebSocket deliveries that arrive with different transient IDs.
            if trade_id in self.seen_trade_ids or fingerprint in self.seen_trade_fingerprints:
                return
