...
def test_duplicate_websocket_payload_with_different_id_is_ignored(tmp_path):
    sim = RealisticFillSimulator(tmp_path / "orders.json")
    sim.place_order(
        condition="c1", market="m1", token="t1", side="Up", target_price=0.20,
        notional=1.0, placed_ts=100.0, window_end_ts=400.0, depth_ahead=0.0,
    )
    sim.on_trade_print("t1", 0.20, 6.0, 110.123456, "id-a", "SELL", "")
    first = sim.drain_fills()
    assert len(first) == 1
    # Same exact economic event, but a different transient event ID.
    sim.on_trade_print("t1", 0.20, 6.0, 110.123456, "id-b", "SELL", "")
    assert sim.drain_fills() == []
    assert sim.metrics(111.0)["filled_notional"] == first[0]["notional"]
