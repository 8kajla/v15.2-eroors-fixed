...
def test_shadow_immediate_ask_ignores_signal_limit_and_consumes_visible_book(tmp_path):
    ...
    s.session = fake
    # The execution budget is derived from shares * the execution ask passed by
    # the planner; the signal limit is not a trigger.
    response = s.adaptive_buy("t1", 0.55, 5.0, "c1")
    assert response["success"] is True
    assert response["status"] == "matched"
    assert response["takingAmount"] == pytest.approx(2.0 + 1.65 / 0.56)
    filled = 2.0 + 1.65 / 0.56
    expected_cost = 2.0 * 0.55 + 1.65
    assert response["vwap"] == pytest.approx(expected_cost / filled)
    assert response["unfilled_shares"] == pytest.approx(5.0 - filled)
...
def test_adaptive_buy_allows_strategy_lot_below_public_min(monkeypatch, tmp_path):
    from shadow_clob import ShadowCLOB

    c = ShadowCLOB(tmp_path / "shadow.json")
    monkeypatch.setattr(c, "book_details", lambda token: {
        "asset_id": token, "market": "m", "min_order_size": 5.0, "tick_size": 0.01,
        "best_bid": 0.19, "best_ask": 0.21, "last_trade_price": 0.2,
        "bids": [{"price": "0.19", "size": "5"}],
        "asks": [{"price": "0.21", "size": "5"}],
    })
    r = c.adaptive_buy("tok", 0.21, 1.0, "cond")
    assert r["takingAmount"] == 1.0
    assert r["makingAmount"] == 0.21


def test_shadow_immediate_ask_caps_cost_when_book_moves_after_plan(tmp_path):
    s = ShadowCLOB(tmp_path / "shadow.json")
    fake = FakeSession()
    fake.book["asks"] = [
        {"price": "0.21", "size": "100"},
    ...
    s.session = fake
    # Planned at $0.18 for 0.518519 shares => immutable budget $0.09333342.
    response = s.adaptive_buy("t1", 0.18, 0.518519, "c1")
    assert response["makingAmount"] <= (0.518519 * 0.18) + 1e-8
    assert response["takingAmount"] <= (0.518519 * 0.18 / 0.21) + 1e-8
