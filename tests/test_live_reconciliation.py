...
def test_live_ledger_closes_partial_fak_and_releases_unfilled_reservation(tmp_path):
    ledger = LiveLedger(tmp_path / "partial-fak.json", 10)
    ledger.record_order("o-fak", "c", "t", "Up", .55, 5.0, "BTC")
    fills = ledger.sync_trades([{
        "id": "shadow-trade-o-fak",
        "status": "CONFIRMED",
        "taker_order_id": "o-fak",
        "size": "3.0",
        "filled_cost": "1.65",
        "price": ".55",
        "fee_rate_bps": "7",
        "order_final_status": "PARTIAL_CANCELED",
    }])
    assert len(fills) == 1
    assert ledger.orders["o-fak"]["status"] == "CLOSED_OR_CANCELED"
