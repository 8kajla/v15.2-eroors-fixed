        ...
        for trade in trades or []:
            ...
            for shares, price, maker in matched_parts:
                ...
                if cost > remaining_reserved + 1e-7:
                    # Shadow execution is a local simulation; the visible ask can
                    # move between planning and reconciliation. Bound the booked
                    # quantity to the immutable V15.2 principal reservation. Real
                    # authenticated CLOB fills remain fail-closed.
                    if order.get("shadow_execution"):
                        allowed_shares = remaining_reserved / price if price > 0 else 0.0
                        if allowed_shares <= 1e-12:
                            continue
                        shares = min(shares, allowed_shares)
                        cost = shares * price
                        if cost > remaining_reserved + 1e-8:
                            continue
                    else:
                        raise RuntimeError(
                ...
                total_cost += cost
                # Principal reservation excludes fees; fees are separate cash cost.
                remaining_reserved -= cost
